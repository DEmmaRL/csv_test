...
class CSV_TO_ODS
    ...
    raise ArgumentError, "Archivo de entrada #{archivo_input} inexistente" unless File.exist?(archivo_input)
    @archivo_output=archivo_output
    @archivo_input=archivo_input
	end
  def crear
    if !File.exist?(@archivo_input)
      return 'Archivo de entrada inexistente'
    end
